...
me apasiona”</span>"
    ...
    public function cargarSlider(){
        $queryItems = "SELECT * FROM `itemslider` WHERE `estado` = 1";
    
        $r_imgslider = $this->_modelo->selectPersonalizado($queryItems);
        if($r_imgslider->fetch(PDO::FETCH_ASSOC)>0){
            foreach ($r_imgslider as $key => $value) {
               echo "<img src='".RECURSOS."img".DS."$value[img]' alt=\"Mercado Heppi\">";
            }
